...
function generateSerializationMethods(fields: any): string {
  return Object.keys(fields)
    ...
    .map((key) => `
  static List<${inferType(fields[key][0], toPascalCase(key))}> ${key}FromMap(List json) {
    return List<${inferType(fields[key][0], toPascalCase(key))}>.from(json.map((e) => ${inferType(fields[key][0], toPascalCase(key))}.fromJson(e)));
  }

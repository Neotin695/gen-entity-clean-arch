  ...
  const disposable = vscode.commands.registerCommand(
    ...
    async () => {
      try {
        ...
        const modelContent = generateModelClass(className, formattedName, fields);
...
@HiveType(typeId: 3)
@AutoMappr([
  MapType<${className}Model, ${className}>(),
])
...
${constructorParams}
  ...
  factory ${className}.fromModel(${className}Model model) =>
      const \$${className}().convert<${className}Model, ${className}>(model);
}`;
}
...
${constructorParams}
  ...
  factory ${className}Model.fromMap(Map<String, dynamic> json) =>
      _\$${className}ModelFromJson(json);

  Map<String, dynamic> toMap() =>    _\$${className}ModelToJson(this);
}`;
}
